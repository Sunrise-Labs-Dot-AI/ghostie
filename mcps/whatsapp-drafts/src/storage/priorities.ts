...
// ~/.whatsapp-mcp/thread-priorities.json (mode 0600), keyed by the
// `thread_jid` string that list_whatsapp_threads/get_whatsapp_thread
// already expose. MIRROR: keep the logic in sync with
// ../../imessage-drafts/src/storage/priorities.ts (which keys by
// String(thread_id) instead).
//
// The menu bar app reads this file DIRECTLY to render its priority queue,
// so the on-disk shape is a LOAD-BEARING contract shared with the Swift side:
//
//   {
//     "schema_version": 1,
//     "priorities": {
...
// (temp + rename, 0600) with symlink guards on both the parent dir and the
// file itself. No daemon involvement: the tool process writes the file
// directly, same trust posture as the witness files.
 ...
 *  MIRROR: keep in sync with ../../imessage-drafts/src/storage/priorities.ts. */
 ...
 *  to "agent" on the next read and the auto-clear logic could no longer tell a
 *  keep-tabs flag apart from a human/agent one. */
...
export function threadPrioritiesFile(): string {
  return PATHS.threadPrioritiesJson;
}

function ensureParentDir(): void {
  const parent = dirname(PATHS.threadPrioritiesJson);
  // Refuse to let mkdirSync(recursive:true) traverse a pre-symlinked
  // ~/.whatsapp-mcp — mirrors the iMessage store's parent guard.
  try {
...
export function loadThreadPriorities(): ThreadPrioritiesFile {
  ...
  const file = PATHS.threadPrioritiesJson;
...
export function setThreadPriority(
  ...
  if (typeof threadJid !== "string" || threadJid.length === 0) {
    throw new Error("thread_jid must be a non-empty string");
  }
  ...
  data.priorities[threadJid] = entry;
  saveThreadPriorities(data);
  return { key: threadJid, entry };
}

/** Remove a thread's priority. Returns whether one existed (idempotent —
 *  clearing an unset thread is a no-op that returns false). */
export function clearThreadPriority(threadJid: string): boolean {
  if (typeof threadJid !== "string" || threadJid.length === 0) {
    throw new Error("thread_jid must be a non-empty string");
  }
  const data = loadThreadPriorities();
  if (!(threadJid in data.priorities)) return false;
  delete data.priorities[threadJid];
  saveThreadPriorities(data);
  return true;
}
