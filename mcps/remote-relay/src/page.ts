...
</style><main><h1>Connect your Mac</h1><p>Read iMessage and WhatsApp, and stage drafts for review in Ghostie.</p>
<div id="login"></div><button id="signup" hidden>Create an account</button><div id="account" hidden>
<p id="identity"></p><p id="details"></p><label id="code-label" hidden>Code shown in Ghostie on your Mac<input id="code" autocomplete="off" maxlength="8" spellcheck="false"></label>
...
<button id="approve" disabled>Connect</button><button id="cancel">Cancel</button><button id="signout">Sign out</button></div><p id="status" role="status" aria-live="polite">Loading secure sign-in...</p></main>
<script nonce="${nonce}">
...
byID('approve').onclick=async()=>{byID('approve').disabled=true;try{
