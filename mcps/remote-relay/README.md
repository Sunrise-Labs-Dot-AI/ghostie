...
5. Approve the client's access in the browser. The consent page lists exactly what the client asked for: reading messages, staging drafts, creating compose links, or any subset. The access token is bound to this exact Mac URL and expires after one hour. The client also receives a refresh token (30 days, replaced on every use) so it renews access without a new approval; replaying a replaced refresh token revokes the whole grant. Disconnect account revokes every token for this Mac. Tokens issued before compose-link consent was added are rejected and must be replaced through this flow.
