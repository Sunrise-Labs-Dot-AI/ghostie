...
5. Approve the client's access in the browser. The consent page lists exactly what the client asked for: reading messages, staging drafts, creating compose links, or any subset. The access token is bound to this exact Mac URL and expires after one hour. The client also receives a refresh token (30 days, replaced on every use) so it renews access without a new approval. A replaced refresh token presented again within 30 seconds is treated as a concurrent refresh and gets another pair; presented later, it is a replay and revokes the whole grant, including its access tokens. Disconnect account revokes every token for this Mac. Tokens issued before compose-link consent was added are rejected and must be replaced through this flow.
...
One Mac-backed tool call reaches the Mac at a time; the relay queues up to eight more per host and answers a waiter with a JSON-RPC busy error after 15 seconds counted from its own arrival (a run of slow calls can therefore time out waiters that did nothing wrong), while discovery and pings run concurrently (eight in flight per host, sixteen per account, one thousand per process). Scopes are enforced per tool at the relay from an explicit allowlist that mirrors the Mac's remote allowlist (a test keeps them equal): `messages:read` for every read, `messages:draft` for `stage_message_draft`, `messages:link` for `ghostie_create_messages_link`; a tool not on the list is refused before it reaches the Mac; discovery omits tools the token cannot call, and a call outside the grant returns 403 with `WWW-Authenticate: Bearer error="insufficient_scope"` naming the scope that tool needs. Six stages/minute on the Mac; six compose-link creations/minute and one in-flight creation per host at the relay; ten link creations in flight globally; 240 MCP requests/minute per host at the relay (fixed one-minute window; a client that initializes several personas in parallel sends about four requests each and does not retry a refusal); 64 KiB request and 1 MiB response limits; 20-second host-response timeout. Link creation makes one five-second upstream attempt and never automatically retries an ambiguous result. Body reads retain existing scoped-history rules and are capped at 100 results per call. Relay or host overload fails closed.

## Service configuration

Run a **single relay process** with a persistent metadata volume. Host routing is process-local, so multiple replicas require a future coordinated routing design. A restart drops connections and outstanding requests; Macs reconnect. Existing hashed tokens, refresh tokens and host records survive; pending pairing and authorization codes expire with the process. Grant scope and family live in `token_grants` and `refresh_tokens` beside the original `tokens` table, so an earlier relay build can still run against the same database if a rollback is ever needed. During such a rollback the old build cannot refresh, and its `/oauth/revoke` and Disconnect do not touch `refresh_tokens`: a refresh token revoked during the window stays valid once this build returns, so after a rollback with any revocation in it, delete every row from `refresh_tokens` before redeploying this build. Rotated refresh tokens are kept for 24 hours so a late replay still revokes the family, then pruned.

The Mac may send `{"heartbeat": <number>}` over its connection; the relay echoes it. A Mac that sends heartbeats can detect a silently dead path within seconds instead of waiting for the operating system.
...
Clients must support static public client IDs, authorization code with PKCE S256, the `resource` parameter at authorization and token exchange, and Streamable HTTP. Clients may request any subset of `messages:read messages:draft messages:link` (plus the conventional `offline_access`, which is accepted and ignored); an absent `scope` requests everything. The token response includes a rotating refresh token; the `refresh_token` grant requires `refresh_token` and `client_id`, accepts an optional `resource` (both must match the grant), and an optional narrower `scope`. A cap hit answers 503 `temporarily_unavailable` and leaves the presented refresh token usable; every other refusal is `invalid_grant`, so the endpoint does not reveal whether a token is live. No dynamic client registration, client secrets, wildcard redirects, or implicit grant. An exact HTTP loopback callback is permitted only when explicitly registered. Dynamic loopback ports are not supported. Validate the intended client with its real callback before launch; this implementation has SDK transport tests, not a certified production-client compatibility list.
...
Tests cover Clerk-session gate injection, cross-user consent refusal, exact redirects, PKCE downgrade, one-use code exchange, old-policy token invalidation, wrong client/resource/proof, expiry, scope subsets and per-tool enforcement, refresh rotation, replay revocation and narrowing, token revocation, cancellation, offline host, stale-connection replacement, concurrent discovery with serialized tool calls, heartbeat echo, disconnection, timeout/no retry, rolling link limits, and synthetic message persistence checks. The production container smoke also exercises the refresh grant and replay revocation. A standard MCP SDK test verifies tool discovery and protocol-valid rate-limit results. The production container smoke invokes the relay-owned link tool against a synthetic local opener while the Mac socket is disconnected. Host SDK tests cover real HTTP client framing, tool visibility, strict fields, local rate enforcement, and response filtering. No personal message data is used.
