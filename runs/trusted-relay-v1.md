...
Rollout completed: fixed-code deployment `2417b980-81d3-4fe7-92e4-ffbc28b56ba1` succeeded with web-only config; final both-callback deployment `b18fb8d0-a3fe-4e42-9cd3-8b5613f61b7b` succeeded and live `/health` returned `{"status":"ok"}`. GitHub relay tests and production-container smoke passed at implementation head `1c965be`; unrelated Swift jobs were still running at handoff. Static auth handoff uses CLIENT_ID `ghostie-cursor`, ordered scopes `["messages:read", "messages:draft"]`, no secret, existing host-specific MCP URL, PKCE S256 and matching resource at authorization/token exchange. No Grok Bot message was sent by the agent; James receives the pasteable configuration to complete the Connect card.

### Live Mac permission check after Grok Bot connection

James reports Grok Bot connection works, then raised concern about the additional local build lacking Full Disk Access. Read-only inspection found both installed PID 1520 and temporary QA PID 16995 running. The QA UI reports remote hosting Online and Full Disk Access granted; macOS Privacy & Security shows Ghostie enabled. The temporary iMessage/WhatsApp/remote-host processes are children of QA PID 16995. A fresh `ghostie_health_check` through the signed QA MCP returned iMessage `open_status: ok` without reading message bodies. Bundle signature verification and installed-app smoke (all three MCP initialize responses) pass. No privacy setting, account, or app installation was changed.

QA Advanced diagnostics still marks helper binaries failed despite signature and launch checks passing. Code inspection suggests the temporary /tmp versus /private/tmp canonical-prefix mismatch in HealthChecks; that diagnosis is not a completed fix. Both copies remain in place pending consolidation with the final app update. Asked whether James had just enabled FDA or Grok Bot still showed an error; do not treat the old three-day invocation witness as current proof. The direct fresh daemon diagnostic above is the current evidence.
